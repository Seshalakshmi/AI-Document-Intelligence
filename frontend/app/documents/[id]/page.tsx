...
export default function DocumentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const extractInvoiceMutation = useMutation({
    mutationFn: () => api.extractInvoiceData(documentId, token ?? undefined),
    onSuccess: () => {
      // Re-extraction rewrites the same row, and ChunkList's structured
      // view reads from it too, so both queries need to refresh.
      queryClient.invalidateQueries({ queryKey: ['document', documentId, 'extracted', token] })
      queryClient.invalidateQueries({ queryKey: ['document', documentId, 'chunks', token] })
    },
    onError: (err: unknown) => alert('Invoice extraction failed: ' + getErrorMessage(err)),
  })
        ...
        <div className="border rounded p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {doc.raw_text && (
                <button
                  onClick={() => extractInvoiceMutation.mutate()}
                  disabled={extractInvoiceMutation.isPending}
                  title="Re-run invoice extraction against the current extraction schema"
